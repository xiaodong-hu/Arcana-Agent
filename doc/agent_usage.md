...
- `[commands.safe]` contains read-only commands that run through AAS without a
  ...
  no-confirmation by default, except paths under project `.arcana/`.
- `.arcana/authorized_prompt.md` is regenerated by the authority daemon and prepended to the LLM system context.

Natural-language requests should be answered using any available combination of
AAS tools, commands, filesystem authority, and network authority that can
materially improve the answer. Temporary scripts should be written under
