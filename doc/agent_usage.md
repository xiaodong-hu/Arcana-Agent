...
├── INSTRUCTION.md              # API-only guidance injected for agents
...
```
my-app/.arcana/
├── authority.toml              # Project authority rules, created on approved registration
...
### 2.3 Authority Context and Requests
...
- `.arcana/authorized_prompt.md` is regenerated by the authority daemon and prepended to the LLM system context.

Natural-language requests that imply running code, inspecting or changing files,
fetching URLs, or using commands should be translated by the model into AAS JSON
requests. Requests to write scripts for concrete inputs are treated as
verification tasks, so the model should run the script through AAS unless the
user explicitly says not to. Temporary scripts should be written under
`.arcana/tmp/`; persistent project files should use the recorded `write` API.

The LLM can request operations through the authority daemon:
...
When an operation needs human review, AAS asks the user to allow, edit, or abort. Approved registrations are written to `.arcana/authority.toml`. Aborts are returned to the LLM as typed errors such as `ToolCallAbortError`, `WebAccessAbortError`, and `FileAccessRegistrationAbortError`; the LLM should report the error and stop that operation.

### 2.4 The Two-Agent Model
